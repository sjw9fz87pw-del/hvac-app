 ...
 *
 * Shared by the CLI seed and the one-shot production bootstrap. The password is
 * a parameter rather than a constant precisely so a public deployment never ends
 * up with the well-known development password on it.
 */
...
const TAG_SECRET_FALLBACK = "dev-only-tag-secret-change-in-production-00000000";

export const DEMO_ACCOUNTS = [
  { email: "admin@clearline.example", role: "Super Admin", sees: "Everything" },
  { email: "manager@clearline.example", role: "Service Manager", sees: "Command center" },
  { email: "tech@clearline.example", role: "Technician", sees: "Today / visits" },
  { email: "owner@monagroup.example", role: "Customer Org Owner", sees: "Both locations" },
  { email: "gm@monagroup.example", role: "Customer Location Manager", sees: "Mona only" },
] as const;
...
export interface SeedResult {
  counts: SeedCounts;
  accounts: { email: string; role: string; sees: string }[];
}
...
/**
 * Wipes and reseeds. Destructive by design - it exists to produce a known,
 * demonstrable state, so callers must be deliberate about when they run it.
 */
export async function seedDemoData(
  ...
  return {
    ...
    accounts: DEMO_ACCOUNTS.map((a) => ({ email: a.email, role: a.role, sees: a.sees })),
  };
}
